...
this.onload = function () {
  ...
  for (i = 0, spanDeg = 0; i < 12; i++, spanDeg += 30) {
    ...
    clock.insertAdjacentHTML(
      "beforeend",
      "<span class='digits d" + i + "'></span>"
    );
    clock.getElementsByClassName("d" + i)[0].style.WebkitTransform =
      "rotate(" + spanDeg + "deg)";
    clock.getElementsByClassName("d" + i)[0].style.transform =
      "rotate(" + spanDeg + "deg)";
    clock.getElementsByClassName("d" + i)[0].style.top =
      radius - 9.5 - top + "px";
    clock.getElementsByClassName("d" + i)[0].style.right =
      radius - 1.5 - right + "px";
  }
  ...
  for (i = 0, spanDeg = 0; i < 60; i++, spanDeg += 6) {
    if (spanDeg % 30 !== 0) {
      ...
      clock.insertAdjacentHTML(
        "beforeend",
        "<span class='mini-digits md" + i + "'></span>"
      );
      clock.getElementsByClassName("md" + i)[0].style.WebkitTransform =
        "rotate(" + spanDeg + "deg)";
      clock.getElementsByClassName("md" + i)[0].style.transform =
        "rotate(" + spanDeg + "deg)";
      clock.getElementsByClassName("md" + i)[0].style.top =
        radius - 4 - top + "px";
      clock.getElementsByClassName("md" + i)[0].style.right =
        radius - 0.5 - right + "px";
    } else continue;
  }
  ...
  ID("my-name").onclick = function () {
    if (clockIsPaused)
      (clockIsPaused = false), loadClock(), setTimeout(startClock, 0);
    else (clockIsPaused = true), clearInterval(start);
  };
};
